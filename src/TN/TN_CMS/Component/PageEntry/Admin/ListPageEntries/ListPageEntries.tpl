    ...
    <form action="" method="GET" class="filter-form mb-4">
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10 col-md-6">
                <small>{$BASE_URL}</small><input type="text" placeholder="filter by URL..." name="path" id="path_filter"
                    value="{$pathFilter}" class="w-100 form-control" />
            </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10 col-md-6">
                <input type="text" placeholder="filter by title..." name="title" id="title_filter"
                    value="{$titleFilter}" class="w-100 form-control" />
            </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10 col-md-6">
                <input type="text" placeholder="filter by tag..." name="tag" id="tag_filter" value="{$tagFilter}"
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10 col-md-6">
                <input type="text" placeholder="mirror site search results..." name="search" id="search_filter"
                    value="{$searchFilter}" class="w-100 form-control" />
            </div>
            ...
            {foreach $contentTypeLabels as $contentType}
                <div class="form-check col-sm-12 col-md-3">
                    <input class="form-check-input content-filter" type="checkbox" value="1"
                        name="{$contentType|replace:' ':'_'}" id="{$contentType|replace:' ':'_'}_filter"
                        {if in_array($contentType, $contentTypeFilters)} checked{/if}>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    {/if}
                    <th colspan="{if !empty($searchFilter)}5{else}3{/if}" class="border-0 table-warning"
                        style="text-align:center !important;">Sort Factor Breakdown</th>
                    {if !empty($searchFilter)}
                        <th colspan="2" class="border-0 table-info" style="text-align:center !important;">Search Selection
                        </th>
                    {/if}
                ...
                {foreach $pageEntries as $pageEntry}
                    ...
                    <tr data-pageEntryId="{$pageEntry->id}" id="pageentry_{$pageEntry->id}"
                        ...
                        <td>{if !empty($pageEntry->thumbnailSrc)}<img src="{$pageEntry->thumbnailSrc}"
                                alt="{$pageEntry->title}" style="max-width:100px; max-height:50px;" />{else}
                        ...
                        {if !empty($searchFilter)}
                            <td>{if !empty($pageEntry->matchedTag)}Tags:
                                    {foreach $pageEntry->matchedTags as $tag}{$tag}
                                        {if !$tag@last}, 
                                        {/if}
                                    {/foreach}
                                {else}Title
                        ...
                        <td class="table-warning">{$pageEntry->timeFactor|round:2}
                            <br /><small>{if $pageEntry->alwaysCurrent}Always
                                up-to-date{else}{$pageEntry->ts|date_format:"%Y-%m-%d %H:$i:%s"}
                                {/if}</small>
                        </td>
