        ...
        {if $articleListing->isArticleEditor || $articleListing->isArticleAuthor}
            <div class="d-flex flex-md-row flex-column">
                <a href="{$BASE_URL}staff/articles/edit?" class="btn btn-primary mb-3 me-0 me-md-3">Add
                    Article From
                    Scratch</a>
                <a data-bs-toggle="modal" data-bs-target="#fromtemplatearticlemodal"
                   class="btn btn-primary template mb-3">Add New Article From Template</a>
            </div>
        {/if}
            ...
            <table class="table">
                    ...
                    <th>
                        <a class="reload-sortable-col-link text-decoration-none text-dark" href="#"
                           data-sortProperty="weight">
                            Weight
                            {if $sortProperty === 'weight' && $sortDirection === SORT_ASC}<i class="bi bi-sort-up"></i>{/if}
                ...
                {foreach $articles as $article}
                    <tr data-articleid="{$article->id}" data-articletitle="{$article->title}">
                        ...
                        <td>{if $article->contentRequired}{$article->contentRequired} {elseif !$article->contentRequired}
                                free{/if}
                        </td>
